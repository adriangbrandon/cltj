...
#include <index/cltj_index_spo_lite_v2.hpp>
...
void query(const std::string &file, const std::string &queries, const uint64_t limit, const uint64_t timeout) {
    ...
    if (result) {
        ...
        for (string &query_string: dummy_queries) {
            ...
            typedef ltj::ltj_iterator_lite_v2<index_scheme_type, uint8_t, uint64_t> iterator_type;
#if ADAPTIVE
...
int main(int argc, char *argv[]) {
    ...
    if (type == "normal") {
        query<cltj::compact_ltj_v2, ltj::util::trait_distinct>(index, queries, limit, timeout);
    } else if (type == "star") {
        query<cltj::compact_ltj_v2, ltj::util::trait_size>(index, queries, limit, timeout);
    } else {
