...
void query(std::vector<cltj::spo_triple> &D, index_scheme_type &graph, const std::string &queries,
           const uint64_t limit, const uint64_t qpu, const uint64_t n) {
...
int main(int argc, char **argv){
    if(argc != 6){
        std::cout<< argv[0] << " <dataset> <queries> <limit> <type> <qpu>" << std::endl;
        return 0;
    }
    ...
    if (type == "normal") {
        query<cltj::compact_dyn_ltj, ltj::util::trait_distinct>(D, index, queries, limit, qpu, n);
    } else if (type == "star") {
        query<cltj::compact_dyn_ltj, ltj::util::trait_size>(D, index, queries, limit, qpu, n);
    } else {
